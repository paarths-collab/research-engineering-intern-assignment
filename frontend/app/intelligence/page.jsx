...
const KeywordSearch = dynamic(
  () => import("@/components/network/KeywordSearch"),
  { ssr: false }
);
...
export default function IntelligencePage() {
  ...
  const [viewMode, setViewMode] = useState("all"); // spread | sources | amplifiers | all
  ...
  const [searchGraphData, setSearchGraphData] = useState(null);
  ...
  useEffect(() => {
    async function fetchGraph() {
      ...
      try {
        ...
        // If a headline (narrative) is selected, filter the graph to that narrative.
        // If an author filter is selected, filter the graph to narratives containing that author.
        if (selectedNarrativeId) q.set("narrative_id", String(selectedNarrativeId));
        if (selectedAuthorFilter) q.set("author", String(selectedAuthorFilter));

        console.log(`[GraphFetch] Requesting mode=${viewMode}...`);
        const res = await fetch(`/api/network/intelligence/graph?${q.toString()}`);
        if (res.ok) {
          const data = await res.json();
          console.log(`[GraphFetch] Received ${data.nodes?.length || 0} nodes, ${data.edges?.length || 0} edges`);
          setGraphData(data);
        } else {
          console.error(`[GraphFetch] Failed: ${res.status}`);
        }
      } catch (e) {
        console.error("[GraphFetch] Error:", e);
      } finally {
        setLoading(false);
      }
    }
    fetchGraph();
  }, [viewMode, minAuthors, minSubreddits, selectedNarrativeId, selectedAuthorFilter]);
  ...
  const handleSearchResults = (data) => {
    ...
    setSearchGraphData({ nodes: data.nodes, edges: data.edges });
  };
  ...
  const handleToggleAuthorFilter = useCallback((author) => {
    const next = String(author || "").trim();
    if (!next) return;
    setSelectedAuthorFilter((prev) => (prev === next ? null : next));
  }, []);
    ...
    if (type === "subreddit") {
      ...
      try {
        ...
        const [detailRes, ecoRes] = await Promise.all([
          fetch(
            `/api/network/intelligence/subreddit/${encodeURIComponent(label)}/details?${q.toString()}`
          ),
          fetch(`/api/network/intelligence/subreddit/${encodeURIComponent(label)}/ecosystem?${q.toString()}`),
        ]);
      ...
      // Redirection logic for domain click
      if (node.metadata?.url) {
        window.open(node.metadata.url, "_blank", "noopener,noreferrer");
      }
      setSubredditInsight({
        name: label,
        description: `Source influence analysis for ${label}.`,
        top_narratives: [],
        stats: { reach: "N/A", authors: "N/A" }
      });
    } else if (type === "author") {
      // Logic for author click
      console.log("Author clicked:", label);
      setSubredditInsight({
        name: `u/${label}`,
        description: `Amplifier network analysis for author ${label}.`,
        top_narratives: [],
        stats: { reach: "N/A", post_count: "N/A" }
      });
    }
  }, [minAuthors, minSubreddits]);
  ...
  const finalGraphData = useMemo(() => {
    // 1. If focused on a specific subreddit (ecosystem view), show only that
    if (focusedGraphData) return focusedGraphData;

    // 2. If a keyword search is active, use the search results
    // The backend now handles viewMode (spread/sources/amplifiers) inside the search endpoint
    if (searchGraphData) return searchGraphData;

    // 3. Otherwise, show the global baseline graph
    return graphData;
  }, [searchGraphData, graphData, focusedGraphData]);
    ...
    <div className="h-screen flex flex-col font-mono" style={{ background: BG, color: "#e2e8f0" }}>
      ...
      <header className="flex-shrink-0 px-8 py-5 flex items-center justify-between"
        ...
        <div className="flex items-center gap-1.5 p-1 bg-white/5 rounded-sm border border-white/10">
            {[
              { id: "all", label: "Ecosystem", color: "#F8FAFC" },
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <section className="flex-1 relative flex flex-col min-w-0" style={{ borderRight: `1px solid ${BORDER}` }}>
            <SectionHeader
              ...
              badge={`${viewMode.toUpperCase()} · authors>=${minAuthors} · subs>=${minSubreddits}${selectedNarrativeId ? " · headline=1" : ""}${selectedAuthorFilter ? ` · author=u/${selectedAuthorFilter}` : ""}${focusedSubreddit ? ` · focus=r/${focusedSubreddit}` : ""}`}
              ...
              <div className="border-b" style={{ borderColor: BORDER }}>
                <SectionHeader 
                  ...
                  actions={(
                    <div className="flex items-center gap-2">
                      {searchGraphData && (
                        <button
                          onClick={() => setSearchGraphData(null)}
                          className="text-[9px] uppercase tracking-wider px-2 py-0.5 border border-red-500/50 text-red-400 hover:bg-red-500/10 transition-colors"
                        >
                          Clear
                        </button>
                      )}
                      ...
                      >
                        {collapsedSections.search ? "Expand" : "Collapse"}
                      </button>
                    </div>
                  )}
                />
                {!collapsedSections.search && (
                  <KeywordSearch 
                    ...
                    viewMode={viewMode}
                    externalStats={graphData?.keyword_stats}
                    externalSources={graphData?.top_sources}
                  />
                )}
              </div>

              <SectionHeader label="Narratives & Trends" accent={ACCENT} />
            ...
            <div className="px-4 py-3 border-b" style={{ borderColor: BORDER }}>
              ...
              {!collapsedSections.amplifiers && (
                ...
                {leaderboard.slice(0, 10).map((u, idx) => (
                  <button
                    key={`${u.author}-${idx}`}
                    onClick={() => handleToggleAuthorFilter(u.author)}
                    className="w-full flex items-center justify-between text-[10px] font-mono px-2 py-1 border transition-colors hover:bg-white/5"
                    style={{
                      borderColor: selectedAuthorFilter === u.author ? "rgba(250,204,21,0.55)" : "rgba(255,255,255,0.10)",
                      background: selectedAuthorFilter === u.author ? "rgba(250,204,21,0.10)" : "transparent",
                    }}
                    title="Click to filter graph by this author"
                  >
                    <span className="truncate text-left" style={{ color: "rgba(255,255,255,0.74)", maxWidth: 180 }}>
                      {idx + 1}. u/{u.author}
                    </span>
