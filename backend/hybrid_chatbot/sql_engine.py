...
class SQLStore:
    ...
    def connect(self) -> duckdb.DuckDBPyConnection:
        if self._conn is None:
            # On platforms like Vercel, the filesystem is read-only.
            # We try read-only first if it fails with the default.
            try:
                self._conn = duckdb.connect(str(self.db_path), read_only=True)
            except Exception:
                # Fallback to default (read-write) for local dev
                self._conn = duckdb.connect(str(self.db_path), read_only=False)
        return self._conn

    def initialize(self) -> None:
        # Check if we are read-only before trying to write
        conn = self.connect()
        is_read_only = False
        try:
            # DuckDB doesn't have a simple 'is_read_only' property on the connection object,
            # but we can check if the file is writable or if we're in a common serverless env.
        ...
        if is_read_only:
            logger.info("Serverless environment detected: skipping SQL table initialization (expecting pre-built DB).")
            return
