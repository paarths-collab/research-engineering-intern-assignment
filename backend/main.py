...
def diagnostic():
    ...
    lateral_query_error = None
    posts_table_error = None
    version = None
    ...
    if db_file.exists():
        try:
            ...
            # Test 1: simple narratives query
            try:
                con.execute("SELECT COUNT(*) FROM narratives").fetchone()
                db_stats = "Narratives: OK"
            except Exception as e:
            ...
            try:
                ...
                db_stats += f", Posts: {count} rows"
            except Exception as e:
                posts_table_error = str(e)
                
            # Test 3: Simple check on subreddit_edges
            try:
                con.execute("SELECT COUNT(*) FROM subreddit_edges").fetchone()
                db_stats += ", Subreddit Edges: OK"
            except Exception as e:
                lateral_query_error = str(e)
    ...
    return {
        ...
        "posts_table_error": posts_table_error,
        "lateral_query_error": lateral_query_error,
        "files_in_data": files
    }
